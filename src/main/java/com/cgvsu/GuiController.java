...
public class GuiController {
    ...
    private static final long MIN_FRAME_NS = 33_000_000L; // ~30 FPS

    private Color polygonBaseColor = Color.GRAY;
    ...
    private boolean hasActiveModels() {
        return sceneModels.stream().anyMatch(SceneModel::isActive);
    }
    ...
    private void drawScene() {
        ...
        if (sceneModels.isEmpty() || !hasActiveModels()) {
            return;
        }
        ...
        // Обработка непрерывного ввода с клавиатуры
        handleContinuousKeyInput();

        // Обновление освещения
        if (lighting != null && renderingModes.isUseLighting()) {
            lighting.update(camera.getPosition(), camera.getTarget(), camera.getViewMatrix());
        }

        // Рендерим все активные модели
        for (SceneModel sceneModel : sceneModels) {
            if (sceneModel.isActive()) {
                // Создаем матрицу трансформации для модели
